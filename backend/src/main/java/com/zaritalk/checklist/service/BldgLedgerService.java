...
import com.zaritalk.checklist.dto.BldgLedgerResponseDto;
...
/**
 * 국토부 건축물대장 전유부 API 서비스
 * 거래 이력 없이 건물에 등록된 전용면적 목록을 조회
 */
@Slf4j
@Service
public class BldgLedgerService {
    ...
    private static boolean isResidential(String mainPurpsCd) {
        if (mainPurpsCd == null || mainPurpsCd.isBlank()) return true; // 코드 없으면 통과
    ...
    /**
     * 법정동코드(bcode)와 지번주소로 전용면적 목록 조회
     */
