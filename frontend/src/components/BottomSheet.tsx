  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-[640px] bg-white rounded-t-[32px] h-[90vh] flex flex-col">
        {/* 헤더 */}
        <div className={`relative flex flex-col items-center w-full px-[22px] pt-[32px] pb-[16px] shrink-0 ${title ? 'border-b border-bg' : ''}`}>
          {title && (
            <span className="text-[18px] font-bold leading-[136%] tracking-[-0.015em] text-text text-center px-8">
              {title}
            </span>
          )}
          <button
            onClick={onClose}
            className="absolute top-[16px] right-[18px] cursor-pointer text-text"
          >
